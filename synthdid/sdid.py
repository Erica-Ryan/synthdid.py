...
def sdid(data: pd.DataFrame, unit, time, treatment, outcome, covariates=None, 
         cov_method="optimized", noise_level=None, eta_omega=None, eta_lambda=1e-6, zeta_omega=None, zeta_lambda=None, omega_intercept=True, lambda_intercept=True, min_decrease=None, max_iter=10000, 
		 sparsify=sparsify_function, max_iter_pre_sparsify=100, lambda_estimate=None, omega_estimate=None,synth=False,did=False,treated_weights=None,weights=None
		):
	# Fast path for fixed-weight bootstrap: skip panel_matrices if pre-processed data provided
	if weights is not None:
		tdf = data
		ttime = np.array(sorted(data.loc[data["tyear"] > 0, "tyear"].unique()))
	else:
	...
	for i, time_eval in enumerate(ttime):
		...
		if covariates is None or cov_method == "projected":
			if weights is not None:
				# fixed-weight path: skip solver, use provided weights directly
				omega_est = weights["omega"][i]
				lambda_est = weights["lambda"][i]
			else:
	...
	weights_out = {"lambda": lambda_estimate, "omega": omega_estimate}
